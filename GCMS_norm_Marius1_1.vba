...
'
    'MsgBox "The last row and col are " & ActiveSheet.Range("A1").End(xlDown).Row & " " & Range("A1").End(xlToRight).Column
    ' MsgBox find_unused_sheetname("Sheet")
    ' MsgBox marius_quality_control_str_judge("XXXAC QC12X")
    ' MsgBox Application.WorksheetFunction.Max(Range(Cells(2, 1), Cells(10, 1)))
    'testvar = marius_quality_control_mean_block(5)
    'MsgBox testvar(1)
    'MsgBox testvar(2)
    'MsgBox testvar(3)
    'MsgBox testvar(4)
    MsgBox marius_quality_control_mean(6)
    
End Sub

Sub Marius_calc_qc_stats1_1()
    ...
    For i = 1 To max_row
        For j = 1 To SampleName_ColNum
            normalized_sheet.Cells(i, j) = Cells(i, j)
        Next j
    Next i
    
    For j = SampleName_ColNum + 1 To max_col
    
        qc_block_mean = marius_quality_control_mean_block(j)
        qc_mean = marius_quality_control_mean(j)
            
        qc_mean_sheet.Cells(1, j - 2) = data_sheet.Cells(1, j)
        qc_mean_sheet.Cells(2, j - 2) = qc_mean
    
        For i = 1 To UBound(qc_block_mean) ' max_block_num
            qc_mean_sheet.Cells(i + 2, j - 2) = qc_block_mean(i)
        Next i
        
        cur_block_num = 1
        normalized_sheet.Cells(1, j) = data_sheet.Cells(1, j)
        For i = 2 To max_row
            If Cells(i, BLOCK_ColNum) <> "" Then
                cur_block_num = Cells(i, BLOCK_ColNum).Value
            End If
            
            normalized_sheet.Cells(i, j) = Cells(i, j) * qc_mean / qc_block_mean(cur_block_num)
...
Function marius_quality_control_mean(ByVal icolnum As Integer) As Double
  ...
  For i = 2 To ActiveSheet.Range("A1").End(xlDown).Row
    ...
    If marius_quality_control_str_judge(Cells(i, SampleName_ColNum)) = True Then
        block_total = block_total + Cells(i, icolnum).Value
...
Function marius_quality_control_mean_block(ByVal icolnum As Integer) As Double()

  Const BLOCK_ColNum As Integer = 2
  Const SampleName_ColNum As Integer = 3
  ...
  max_block_num = Application.WorksheetFunction.Max(Range(Cells(BLOCK_ColNum, 2), _
                                                          Cells(ActiveSheet.Range("A1").End(xlDown).Row, BLOCK_ColNum)))
  ...
  For i = 2 To ActiveSheet.Range("A1").End(xlDown).Row
    If Cells(i, BLOCK_ColNum).Value = "" Then
